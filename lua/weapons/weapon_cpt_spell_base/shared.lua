...
function SWEP:PrimaryAttack(ShootPos,ShootDir)
	...
	timer.Simple(self.SpellDelay,function() if IsValid(self) then self.IsFiring = false self.CanUseIdle = true end end)
	timer.Simple(self.SpellDelay +0.001,function() if IsValid(self) then self:DoIdleAnimation() end end)
end
---------------------------------------------------------------------------------------------------------------------------------------------
function SWEP:CreateSummon(pos,ent)
	local ent = ents.Create(ent)
	ent:CPT_SetClearPos(pos)
	ent:SetAngles(Angle(0,self.Owner:GetAngles().y,self.Owner:GetAngles().r))
	ent:CPT_SetSummonedByPlayer(self.Owner)
	ent:Spawn()
	ent:Activate()
	ent:CPT_SetSummonedByPlayer(self.Owner)
