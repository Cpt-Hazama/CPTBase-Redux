...
function TOOL:LeftClick(tr)
	...
	if tr.Entity && tr.Entity:IsValid() && IsValid(tr.Entity) then
		...
		if ent:GetNWString("CPTBase_NPCFaction") == nil then
			if ent:IsNPC() then
				ent:SetNWString("CPTBase_NPCFaction","FACTION_NONE")
			else
				ent:SetNWString("CPTBase_NPCFaction","FACTION_PLAYER")
			end
		end
		local oldFaction = ent:GetNWString("CPTBase_NPCFaction")
		-- local newFaction = self.Faction
		local newFaction = self.ClientConVar["cpt_tool_faction"]
		ent:SetNWString("CPTBase_NPCFaction",newFaction)
...
function TOOL:RightClick(tr)
	...
	if tr.Entity && tr.Entity:IsValid() && IsValid(tr.Entity) && tr.Entity:IsNPC() then
		local ent = tr.Entity
		print(ent:GetNWString("CPTBase_NPCFaction"))
		if ent:GetNWString("CPTBase_NPCFaction") != nil then
			self.ClientConVar["cpt_tool_faction"] = ent:GetNWString("CPTBase_NPCFaction")
			-- self.Faction = ent:GetNWString("CPTBase_NPCFaction")
		else
...
function TOOL:Reload(tr)
	...
	-- self:GetOwner():SetNWString("CPTBase_NPCFaction",self.Faction)
	self:GetOwner():SetNWString("CPTBase_NPCFaction",self.ClientConVar["cpt_tool_faction"])
