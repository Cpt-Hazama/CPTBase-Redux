...
function ENT_Meta:ZelusPlaySound(sSound,hSNDLV) --This is Zelus's code to prevent his SNPCs from generating errors
	if hSound == "Die" || hSound == "Death" then --Doesn't work...? Just cuts off before the SNPC dies. Wanker.
...
function NPC_Meta:PlaySound(_Sound,_SoundLevel,_SoundVolume,_SoundPitch,_UseDotPlay)
	...
	if self:GetNWBool( "bZelusSNPC" ) == true then return self:ZelusPlaySound(_Sound,_SoundLevel) end
...
function ENT_Meta:ZelusMoveToPosition(hPos) --Done, his SNPCs won't spam errors now
...
function NPC_Meta:MoveToPosition(pos)
	if self:GetNWBool( "bZelusSNPC" ) == true then return self:ZelusMoveToPosition(pos,bChase) end
	local tr = util.TraceLine({
